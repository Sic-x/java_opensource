...
$(function () {
    ...
    //把常用的一些组件获取
    var  roleDataGrid = $("#roleDataGrid"); //页面显示表格
   ...
   //只要有data-method属性的元素我都要为它注册事件
    ...
    itsource = {
        ...
        add(){
            //密码显示并启用
           // $(window.top.document).find("#mtree").tree('reload');
            ...
            editForm.form('submit', {
                ...
                success:function(data){
                    ...
                    if(result.success){
                        ...
                        /*
                            role.js里面是不能写方法控制其他窗体的组件，
                            需要通过顶层窗口去调用其他窗体内的js方法
                            refreshTree()为树.js（index.js）的刷新树的方法
                         */
                ...
                if (r){
                    ...
                    $.get("/role/delete",{id:row.id},function (result) {
                        if(result.success){
                            ...
                            //删除失败，给出失败提示
                            $.messager.alert('提示','删除没有成功，原因是:'+result.msg,"info");
                        }
                    })
                }
            });
    ...
    rolePermsGrid.datagrid({
        fit:true,
        fitColumns:true,
        onClickRow:itsource.removePerms,
        columns:[[
