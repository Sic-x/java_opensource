...
import com.xmh.aisell.query.SystemDictionaryTypeQuery;
...
public class SystemDictionaryTypeController {
    ...
    public String systemDictionaryTypeIndex(){
    ...
    public UIPage<SystemDictionaryType> page(SystemDictionaryTypeQuery query){
        ...
        //List content = systemDictionaryTypeService.queryPage(query).getContent();
        return new UIPage(systemDictionaryTypeService.queryPage(query));
    }

    /**
     * @param id 前台传入的id字符串
     *        调用systemDictionaryTypeService.delete
    ...
    /**
     * 前台会传相应的数据(SystemDictionaryType)过来
    ...
    /**
     * 更新前方法，前台发送请求/systemDictionaryType/update?cmd=update
     * 只匹配更新请求通过发送过来的id从数据库获取相对应的SystemDictionaryType持久化对象
    ...
    /**
     *
     * @param systemDictionaryType 传入参数systemDictionaryType现在是从beforeUpdate方法拿的
     *        前台依旧会传过来与数据库中持久化对象systemDictionaryType不同的修改数据
     ...
     *  执行systemDictionaryTypeService.save(systemDictionaryType);方法
     *
    ...
    /**
     * 删除方法
     * @param id 根据 SystemDictionaryType id 删除员工
     * @return
     */
