//所有 product 的 js 代码抽取到 product.js
//入口函数：读取所有页面元素再执行
$(function () {
            ...
            if(row){
                ...
                $.messager.confirm('确认对话框', '确定要修改吗?', function(r){
                    ...
                    if (r){
                        ...
                        if(row.productType){
                            //ecombox.combobox("select",row.productType.id);
...
function productTypeFormat(value){
    ...
    return value?value.name:"没有产品类型";

};
function unitFormat(value){

    return value?value.name:"没有单位";

};

function brandFormat(value){

    return value?value.name:"没有品牌";

};

function colorFormat(value){

    return `<div style="background-color: ${value};width: 30px;height: 30px"></div>`;
